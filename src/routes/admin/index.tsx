...
import {
  ...
  Clock, AlertCircle, ArrowUpRight, ArrowDownRight, Activity,
  Leaf, UserPlus, ShoppingCart, CreditCard, Award
} from "lucide-react";
import { Badge } from "../../components/ui/badge";
import {
  adminStats, revenueData, categoryPerformance, activityFeed,
  orders, farmers, products, formatRWF, getStatusColor
} from "../../lib/admin-data";
  ...
  return (
    <div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100 hover:shadow-md transition-all duration-200 group">
      <div className="flex items-start justify-between">
        <div
          className={`w-11 h-11 rounded-xl flex items-center justify-center ${color} shadow-sm group-hover:scale-105 transition-transform`}
        >
...
function AdminDashboard() {
  ...
  const newFarmers = farmers.filter((f) => f.status === "pending").slice(0, 4);
  const pendingProducts = products.filter((p) => p.status === "pending").slice(0, 3);
  ...
  return (
    <div className="p-6 space-y-6 max-w-[1600px]">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">
            Deacomart Ltd — Be EcoWise · Rwanda Agribusiness Ecosystem
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-6 gap-4">
        <StatCard icon={Leaf} label="Registered Farmers" value={adminStats.totalFarmers.toLocaleString()} trend="+12%" trendUp color="bg-gradient-to-br from-emerald-500 to-emerald-700" />
        <StatCard icon={Users} label="Total Buyers" value={adminStats.totalBuyers.toLocaleString()} trend="+8%" trendUp color="bg-gradient-to-br from-teal-500 to-teal-700" />
        <StatCard icon={Package} label="Active Products" value={adminStats.activeProducts.toLocaleString()} trend="+5%" trendUp color="bg-gradient-to-br from-blue-500 to-blue-700" />
        <StatCard icon={ShoppingCart} label="Orders Today" value={adminStats.ordersToday} trend="+18%" trendUp color="bg-gradient-to-br from-violet-500 to-violet-700" />
        <StatCard icon={DollarSign} label="Monthly Revenue" value={formatRWF(adminStats.monthlyRevenue)} sub="RWF 68.5M" trend="+22%" trendUp color="bg-gradient-to-br from-orange-500 to-orange-600" />
        <StatCard icon={MapPin} label="Districts Covered" value={`${adminStats.districtsCovered}/30`} sub="All Rwanda" trend="+2" trendUp color="bg-gradient-to-br from-rose-500 to-rose-600" />
        <StatCard icon={GraduationCap} label="Farmers Trained" value={adminStats.farmersTrained.toLocaleString()} trend="+15%" trendUp color="bg-gradient-to-br from-indigo-500 to-indigo-700" />
        <StatCard icon={Briefcase} label="Consultancy Requests" value={adminStats.consultancyRequests} trend="+4" trendUp color="bg-gradient-to-br from-cyan-500 to-cyan-700" />
        <StatCard icon={Handshake} label="Active Partnerships" value={adminStats.activePartnerships} trend="+3" trendUp color="bg-gradient-to-br from-amber-500 to-amber-600" />
        <StatCard icon={Truck} label="Delivery Performance" value={`${adminStats.deliveryPerformance}%`} trend="+1.2%" trendUp color="bg-gradient-to-br from-green-600 to-green-800" />
        <StatCard icon={Star} label="Customer Satisfaction" value={`${adminStats.customerSatisfaction}/5`} trend="+0.2" trendUp color="bg-gradient-to-br from-yellow-500 to-orange-500" />
        <StatCard icon={AlertCircle} label="Pending Approvals" value={adminStats.pendingApprovals} sub="Products & farmers" color="bg-gradient-to-br from-red-500 to-red-600" />
      </div>
        ...
        <div className="xl:col-span-2 bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-sm text-gray-400">Monthly revenue trend — 2024</p>
            </div>
        ...
        <div className="xl:col-span-2 bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
          <div className="flex items-center justify-between mb-5">
            <div>
              ...
              <p className="text-sm text-gray-400">Products sold per category</p>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="xl:col-span-2 bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
          <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
            ...
            <Link to="/admin/marketplace/orders" className="text-xs text-emerald-600 hover:text-emerald-700 font-medium">
              View all →
            </Link>
          </div>
              ...
              <tbody className="divide-y divide-gray-50">
                {recentOrders.map((order) => (
                  <tr key={order.id} className="hover:bg-gray-50/50 transition-colors">
                    ...
                    <td className="px-4 py-3 text-xs text-gray-600 max-w-[140px] truncate">{order.buyer}</td>
                    <td className="px-4 py-3 text-xs text-gray-500 max-w-[120px] truncate">{order.product}</td>
                    <td className="px-4 py-3 text-xs font-semibold text-gray-800 text-right">{formatRWF(order.total)}</td>
            ...
            <div className="space-y-2.5">
              {newFarmers.slice(0, 2).map((f) => (
                <div key={f.id} className="flex items-center justify-between py-2 border-b border-gray-50 last:border-0">
                  ...
                  <button className="text-[10px] text-emerald-600 border border-emerald-200 hover:bg-emerald-50 px-2 py-1 rounded font-medium transition-colors">
                    Review
                  </button>
                </div>
              ))}
              {pendingProducts.slice(0, 2).map((p) => (
                <div key={p.id} className="flex items-center justify-between py-2 border-b border-gray-50 last:border-0">
                  ...
                  <button className="text-[10px] text-blue-600 border border-blue-200 hover:bg-blue-50 px-2 py-1 rounded font-medium transition-colors">
                    Approve
                  </button>
                </div>
              ))}
            </div>
            <Link to="/admin/marketplace/products" className="block text-center text-xs text-emerald-600 mt-3 font-medium">
              View all pending →
            </Link>
          </div>
            ...
            <div className="grid grid-cols-3 gap-2">
              {quickActions.map((action) => (
                <Link
                  key={action.label}
                  to={action.path}
                  className={`flex flex-col items-center gap-1.5 p-2.5 rounded-xl text-white text-center transition-all hover:scale-105 active:scale-95 ${action.color}`}
                >
      ...
      {/* Rwanda Geographic Coverage Mini-Panel */}
      <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
        <div className="flex items-center justify-between mb-4">
          <div>
          ...
          <Link to="/admin/geographic" className="text-xs text-emerald-600 hover:text-emerald-700 font-medium">
            Full Map View →
          </Link>
        </div>
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          {[
            { province: "Kigali City", farmers: 620, districts: 3, color: "from-emerald-500 to-emerald-600" },
            { province: "Northern Province", farmers: 1240, districts: 5, color: "from-teal-500 to-teal-600" },
            { province: "Southern Province", farmers: 1180, districts: 8, color: "from-blue-500 to-blue-600" },
            { province: "Eastern Province", farmers: 1050, districts: 7, color: "from-violet-500 to-violet-600" },
            { province: "Western Province", farmers: 782, districts: 7, color: "from-orange-500 to-orange-600" },
          ].map((p) => (
            <div key={p.province} className="bg-gradient-to-br rounded-xl overflow-hidden">
              ...
              <div className="p-3 bg-gray-50">
                ...
                <div className="text-[10px] text-gray-400">{p.districts} districts</div>
              </div>
            </div>
          ))}
        </div>
